...
//        growLabel.mouseLeave().subscribe(growAnimation::backwards);
        ...
        var dropdown = this.model.expandTemplate(DropdownComponent.class, "epic-dropdown", Map.of());
        rootComponent.childById(ButtonWidget.class, "dropdown-button").onPress(button -> {
            rootComponent.child(dropdown);
            dropdown.positioning(Positioning.absolute(button.x() + (button.width() - dropdown.width()) / 2, button.y() + button.height()));
        });
    }
}